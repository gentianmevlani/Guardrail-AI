...
`;
  ...
  // Anti-hallucination: Database Schema Rules
  const ah = analysis.antiHallucination || {};
  if (ah.databaseSchema?.tables?.length > 0) {
    rules["database-schema"] = `---
description: Database schema reference - PREVENTS TABLE/COLUMN HALLUCINATIONS
globs: ["**/*.{ts,tsx,js,jsx}", "**/schema.ts", "**/db/**"]
alwaysApply: true
---

# Database Schema Reference (${ah.ormType || "ORM"})

## ⚠️ CRITICAL: Only these tables exist in \`${ah.databaseSchema.schemaFile || "schema file"}\`

### Tables (${ah.databaseSchema.tables.length})
${ah.databaseSchema.tables.map(t => `- \`${t}\``).join("\n")}

${Object.keys(ah.databaseSchema.columns || {}).length > 0 ? `### Table Columns
${Object.entries(ah.databaseSchema.columns).slice(0, 10).map(([table, cols]) => 
  `**${table}**: ${cols.slice(0, 8).join(", ")}${cols.length > 8 ? "..." : ""}`
).join("\n")}
` : ""}

## 🚫 NEVER INVENT
- New tables not listed above
- New columns not in ${ah.databaseSchema.schemaFile || "schema"}
- Different column names or data types

## Query Pattern (${ah.ormType || "ORM"})
${ah.ormType === "Drizzle" ? `\`\`\`typescript
import { db } from '../db';
import { tableName } from '@/shared/schema';
import { eq } from 'drizzle-orm';

const result = await db.select().from(tableName).where(eq(tableName.id, id));
\`\`\`` : ah.ormType === "Prisma" ? `\`\`\`typescript
...
`;
  ...
  // Anti-hallucination: API Endpoints Rules
  if (ah.apiRouteFiles?.length > 0) {
    const routesByType = {};
    ah.apiRouteFiles.forEach(r => {
      if (!routesByType[r.type]) routesByType[r.type] = [];
      routesByType[r.type].push(r);
    });

    rules["api-endpoints"] = `---
description: API endpoint reference - PREVENTS ENDPOINT HALLUCINATIONS
globs: ["**/routes/**", "**/api/**", "**/*.ts"]
alwaysApply: false
---
...
## ⚠️ All routes are in verified files (${ah.apiRouteFiles.length} files)

${Object.entries(routesByType).map(([type, routes]) => `### ${type === "express" ? "Express Routes" : type === "nextjs-app" ? "Next.js App Router" : "Next.js Pages API"}
${routes.slice(0, 20).map(r => `- \`${r.basePath}\` → \`${r.file}\``).join("\n")}
${routes.length > 20 ? `\n... and ${routes.length - 20} more` : ""}
`).join("\n")}

## 🚫 NEVER
- Invent endpoints not listed here
- Create routes in wrong directories
- Skip input validation
`;
  }

  // Anti-hallucination: UI Components Rules
  if (ah.uiLibrary?.name) {
    rules["components-ui"] = `---
description: UI component reference - PREVENTS COMPONENT HALLUCINATIONS
...
## ⚠️ This project uses ${ah.uiLibrary.name}${ah.uiLibrary.styling ? ` + ${ah.uiLibrary.styling}` : ""}

### Tech Stack
| Category | Library |
|----------|---------|
| UI Components | ${ah.uiLibrary.name || "Custom"} |
| Icons | ${ah.uiLibrary.iconLib || "N/A"} |
| Styling | ${ah.uiLibrary.styling || "CSS"} |
| Toast | ${ah.uiLibrary.toast || "N/A"} |
| Forms | ${ah.uiLibrary.forms || "N/A"} |

${ah.uiLibrary.components?.length > 0 ? `### Available Components
${ah.uiLibrary.components.slice(0, 20).map(c => `- \`${c}\``).join("\n")}
` : ""}
...
${ah.uiLibrary.iconLib !== "Heroicons" ? "- Heroicons" : ""}
${ah.uiLibrary.toast !== "React Hot Toast" ? "- react-hot-toast" : ""}
${ah.uiLibrary.forms !== "Formik" ? "- Formik" : ""}
`;
  }

  // Anti-hallucination: Master Prevention Guide
  rules["hallucination-prevention"] = `---
description: Master anti-hallucination reference - READ THIS FIRST
globs: ["**/*"]
alwaysApply: true
---

# 🛡️ Hallucination Prevention Guide
...
### Database Schema
- ✅ **Schema file**: \`${ah.databaseSchema?.schemaFile || "Not detected"}\`
- ❌ **Never invent tables or columns**

### API Routes
${ah.apiRouteFiles?.length > 0 ? `- ✅ **Route location**: \`${ah.apiRouteFiles[0]?.file?.split("/")[0] || "routes"}/\`` : "- Routes not detected"}
- ❌ **Never invent endpoints**

### Custom Hooks
${ah.customHooks?.length > 0 ? `Available: ${ah.customHooks.slice(0, 10).map(h => `\`${h}\``).join(", ")}` : "None detected"}

## Common Hallucination Patterns to Avoid

1. **Inventing database tables** → Check schema file
2. **Using wrong state management** → Use ${ah.stateManagement || "project's solution"}
3. **Wrong UI library imports** → Use ${ah.uiLibrary?.name || "project's UI"}
4. **Inventing API endpoints** → Check routes directory
5. **Wrong icon imports** → Use ${ah.uiLibrary?.iconLib || "project's icons"}
