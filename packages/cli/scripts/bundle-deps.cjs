...
async function bundlePackage(pkg) {
  ...
  try {
    ...
    await esbuild.build({
      ...
      plugins: [
        {
          name: 'stub-ship-notification',
          setup(build) {
            build.onResolve({ filter: /ship-notification-service/ }, (args) => {
              const normalized = args.path.replace(/\\/g, '/');
              if (normalized.includes('apps/api')) {
                return {
                  path: path.join(__dirname, 'stubs', 'ship-notification-stub.cjs'),
                };
              }
              return undefined;
            });
      ...
      external: [
        ...
        'url',
        'child_process',
        // Optional / server-only deps pulled by @guardrail/core (dynamic imports)
        'redis',
        '@prisma/client',
        '@guardrail/database',
        '.prisma/client',
