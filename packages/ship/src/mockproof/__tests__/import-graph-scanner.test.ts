...
describe("ImportGraphScanner", () => {
        ...
        violations: [
          {
            ...
            message:
              "MockProvider should not be reachable from production entrypoints",
          },
        ],
        scannedFiles: 42,
  ...
  describe("banned patterns", () => {
    it("should have default banned patterns", () => {
      ...
      expect(
        config.bannedImports.some((p: any) => p.pattern === "MockProvider"),
      ).toBe(true);
      expect(
        config.bannedImports.some((p: any) => p.pattern.includes("localhost")),
      ).toBe(true);
    });
    ...
    it("should allow custom banned patterns", () => {
      ...
      expect(
        config.bannedImports.some((p: any) => p.pattern === "MyCustomMock"),
      ).toBe(true);
    });
  });
});
