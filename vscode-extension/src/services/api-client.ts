...
<<<<<<< HEAD
   ...
   * (default https://guardrailai.dev) so users complete linking on the deployed site.
   */
  private normalizeDeviceVerificationUrl(verificationUrl: string): string {
    const webApp = getGuardrailWebAppUrl();
    try {
      const u = new URL(verificationUrl);
      if (u.pathname.includes('link-device')) {
        const web = new URL(webApp);
        return `${web.origin}${u.pathname}${u.search}`;
      }
    } catch {
      /* keep original */
    }
