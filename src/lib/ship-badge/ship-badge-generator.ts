...
export class ShipBadgeGenerator {
   ...
   */
  private calculateVerdict(checks: ShipCheck[]): { verdict: 'ship' | 'no-ship' | 'review'; score: number } {
   ...
   */
  private generateAllBadges(checks: ShipCheck[], verdict: 'ship' | 'no-ship' | 'review', score: number): ShipBadges {
   ...
   * Create SVG badge
   */
  private createBadge(label: string, value: string, color: string, style: 'flat' | 'for-the-badge' = 'flat'): string {
   ...
   */
  private generateEmbedCode(projectId: string, verdict: string, projectName: string): string {
...
**${projectName}** verified by [guardrail](https://guardrailai.dev) - Stop shipping pretend features.`;
   ...
   */
  private async saveBadges(badges: ShipBadges, outputDir: string): Promise<void> {
