...
exports.realityScanner = exports.RealityScanner = exports.DEFAULT_FAKE_PATTERNS = void 0;
const { STRIPE_PK_TEST_PREFIX, STRIPE_TEST_PREFIX, stripeTestSkPkTestOrGenericTestKeyRegex, } = require('../../../bin/runners/lib/stripe-scan-patterns.js');
...
const FAKE_RESPONSE_PATTERNS = [
    ...
    { pattern: new RegExp(STRIPE_PK_TEST_PREFIX, 'i'), name: 'Test Stripe public key' },
    ...
    { pattern: /"id":\s*("demo"|"test"|"fake"|1234567890)/i, name: 'Fake ID pattern' },
...
const SILENT_FALLBACK_PATTERNS = [
    { pattern: /"error":\s*null.*"data":\s*\[\]/i, name: 'Empty success on error' },
    { pattern: /catch.*return\s*\{\s*success:\s*true/i, name: 'Success on catch' },
...
`;
    ...
    getEvidence(item, pattern) {
        if (item.type === 'request') {
            ...
            if (pattern.id === 'demo-response-data' && item.body) {
                const matched = FAKE_RESPONSE_PATTERNS.find(({ pattern }) => pattern.test(item.body));
    ...
    generateReport(result) {
        ...
        lines.push('╚══════════════════════════════════════════════════════════════╝');
        lines.push('');
        const verdictEmoji = result.verdict === 'real' ? '✅' : result.verdict === 'fake' ? '❌' : '⚠️';
        ...
        if (result.verdict === 'real') {
            ...
            lines.push('   No fake data, mock APIs, or placeholder content detected.');
        }
        else {
    ...
    generateDefaultClickPaths() {
        return [
            ...
            // Main actions
            ['button[type="submit"]', '.cta-button', '[data-testid="primary-action"]'],
            // Dashboard/billing
            ['[href*="dashboard"]', '[href*="billing"]', '[href*="settings"]'],
            // Checkout flow
            ['[data-testid="add-to-cart"]', '[data-testid="checkout"]', '[data-testid="pay"]'],
        ];
    }
}
