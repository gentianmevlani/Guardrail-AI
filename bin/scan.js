...
---
*Generated by [guardrail](https://guardrailai.dev)*\`;
...
function copyBadge() {
  const badge = \`[![guardrail Score](https://img.shields.io/badge/guardrail-${result.score}%2F100-${result.score >= 70 ? "green" : result.score >= 40 ? "yellow" : "red"})](https://guardrailai.dev)\`;
...
<div style="background: linear-gradient(135deg, rgba(168, 85, 247, 0.1), rgba(236, 72, 153, 0.1)); border: 1px solid rgba(168, 85, 247, 0.3); border-radius: 12px; padding: 16px; margin-top: 24px; text-align: center;">
  ...
  <a href="https://guardrailai.dev/pricing" style="display: inline-block; margin-top: 12px; padding: 8px 16px; background: linear-gradient(135deg, #a855f7, #ec4899); color: white; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 13px;">Upgrade to Pro →</a>
</div>
`;
...
async function runScan(projectPath) {
  ...
  if (!rateLimit.allowed) {
    ...
    console.log(`  ${c.cyan}→ https://guardrailai.dev/pricing${c.reset}`);
    console.log();
    process.exit(0);
  }
