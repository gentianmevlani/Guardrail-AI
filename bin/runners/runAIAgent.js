...
async function fillAuthForm(page, formType, config, results) {
  ...
  // Test data
  const testData = {
    email: `test-${timestamp}@guardrail.dev`,
      ...
      for (const input of inputs) {
        try {
          ...
          if (await input.isVisible()) {
            ...
            switch (type) {
              case 'email':
                value = `test-${Date.now()}@guardrail.dev`;
                break;
              case 'password':
                value = 'TestPass123!';
                break;
...
<body>
  <div class="container">
    ...
    <div class="footer">
      Generated by guardrail AI Agent • <a href="https://guardrail.dev" style="color:#3b82f6">guardrail.dev</a>
    </div>
  </div>
  <script>
