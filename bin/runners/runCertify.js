...
function parseCertifyArgv(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a.startsWith("--output=")) out.output = a.split("=", 2)[1];
    else if (a === "--embed") out.embed = true;
    else if (a === "--style" && argv[i + 1]) out.style = argv[++i];
    else if (a.startsWith("--style=")) out.style = a.split("=", 2)[1];
  }
  return out;
}

async function runCertify(argv, projectPath) {
  const args = parseCertifyArgv(argv);
  if (args.help) {
    console.log(`
${COLORS.cyan}guardrail certify${COLORS.reset}

  Generate certification badges from the latest scan score.

${COLORS.dim}Usage:${COLORS.reset}
  guardrail certify [--output md|json|all] [--embed] [--style flat|plastic]

${COLORS.dim}Note:${COLORS.reset} Requires a successful scan (run ${COLORS.cyan}guardrail scan${COLORS.reset} first).
`);
    return 0;
  }
