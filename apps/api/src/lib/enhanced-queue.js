...
exports.queueSystem = exports.JobTypes = exports.QueueSystem = void 0;
exports.getQueueSystem = getQueueSystem;
...
// Queue system class
class QueueSystem extends events_1.EventEmitter {
    config;
    connection;
    queues = new Map();
    workers = new Map();
    ...
    constructor(config = {}) {
        ...
        this.connection = new ioredis_1.default(this.config.redis);
    }
    // Create a new queue
    createQueue(name, options = {}) {
        ...
        enhanced_logger_1.logger.info({ queueName: name }, 'Queue created');
        return queue;
    }
    ...
    async registerProcessor(queueName, processor, concurrency = this.config.concurrency.default) {
        ...
        worker.on('completed', (job) => {
            enhanced_logger_1.logger.debug({ jobId: job.id, queue: queueName }, 'Job completed');
        });
        worker.on('failed', (job, err) => {
            enhanced_logger_1.logger.error({ jobId: job?.id, queue: queueName, error: err.message }, 'Job failed');
        });
        worker.on('error', (err) => {
            enhanced_logger_1.logger.error({ queue: queueName, error: err.message }, 'Worker error');
        });
        enhanced_logger_1.logger.info({ queueName, concurrency }, 'Job processor registered');
    }
    // Add a job to the queue
    async addJob(queueName, jobConfig) {
        const queue = this.queues.get(queueName);
        if (!queue) {
            throw new Error(`Queue ${queueName} not found`);
        }
        try {
            ...
            enhanced_logger_1.logger.info({ queueName, jobId: job.id, jobName: jobConfig.name }, 'Job added to queue');
            return job;
        }
        catch (error) {
            enhanced_logger_1.logger.error({ queueName, error }, 'Failed to add job to queue');
            throw new enhanced_error_handler_1.ExternalServiceError('Queue', `Failed to add job: ${error}`, error);
        }
    }
    // Get job status
    async getJobStatus(queueName, jobId) {
        ...
        try {
            ...
            enhanced_logger_1.logger.error({ queueName, jobId, error }, 'Failed to get job status');
            return null;
        }
    }
    ...
    async getQueueStats(queueName) {
        ...
        try {
            ...
            enhanced_logger_1.logger.error({ queueName, error }, 'Failed to get queue stats');
            return null;
        }
    }
    ...
    async pauseQueue(queueName) {
        ...
        await queue.pause();
        enhanced_logger_1.logger.info({ queueName }, 'Queue paused');
    }
    // Resume a queue
    async resumeQueue(queueName) {
        ...
        await queue.resume();
        enhanced_logger_1.logger.info({ queueName }, 'Queue resumed');
    }
    // Clear a queue
    async clearQueue(queueName) {
        ...
        await queue.drain();
        enhanced_logger_1.logger.info({ queueName }, 'Queue cleared');
    }
    // Remove a job
    async removeJob(queueName, jobId) {
        ...
        try {
            ...
            await job.remove();
            enhanced_logger_1.logger.info({ queueName, jobId }, 'Job removed');
            return true;
        }
        catch (error) {
            enhanced_logger_1.logger.error({ queueName, jobId, error }, 'Failed to remove job');
            return false;
        }
    }
    // Retry a failed job
    async retryJob(queueName, jobId) {
        ...
        try {
            ...
            await job.retry();
            enhanced_logger_1.logger.info({ queueName, jobId }, 'Job retried');
            return true;
        }
        catch (error) {
            enhanced_logger_1.logger.error({ queueName, jobId, error }, 'Failed to retry job');
            return false;
        }
    }
    // Schedule a recurring job
    async scheduleRecurringJob(queueName, jobConfig, cronExpression) {
        const queue = this.queues.get(queueName);
        if (!queue) {
            throw new Error(`Queue ${queueName} not found`);
        }
        ...
        enhanced_logger_1.logger.info({ queueName, jobId: job.id, cron: cronExpression }, 'Recurring job scheduled');
        return job;
    }
            ...
            try {
                ...
                for (const [queueName, stats] of Object.entries(allStats)) {
                    if (stats.errorRate > 0.1) { // 10% error rate
                        enhanced_logger_1.logger.warn({ queueName, errorRate: stats.errorRate }, 'High queue error rate');
                    }
                    if (stats.active > this.config.concurrency.max) {
                        enhanced_logger_1.logger.warn({ queueName, activeJobs: stats.active }, 'High queue concurrency');
                    }
                }
            }
            catch (error) {
                enhanced_logger_1.logger.error({ error }, 'Queue monitoring error');
            }
        }, this.config.monitoring.interval);
        enhanced_logger_1.logger.info('Queue monitoring started');
    }
    // Stop monitoring
    stopMonitoring() {
        if (this.monitoringTimer) {
        ...
        enhanced_logger_1.logger.info('Queue monitoring stopped');
    }
    // Graceful shutdown
    async shutdown() {
        enhanced_logger_1.logger.info('Shutting down queue system...');
        this.stopMonitoring();
        // Close all workers
        ...
        enhanced_logger_1.logger.info('Queue system shutdown complete');
    }
    // Helper methods
    getJobStatusFromState(job) {
    ...
    updateJobStatus(queueName, jobId, status, data) {
...
exports.queueSystem = getQueueSystem();
