...
export async function githubRoutes(fastify: FastifyInstance) {
  // ── GitHub App Installation (native, no tokens needed) ──

  /**
   * GET /app/install — Redirect to GitHub App installation page.
   * This is the recommended flow: users install the GitHub App on their
   * org/account, granting Guardrail native access without personal tokens.
   */
  fastify.get(
    "/app/install",
    async (request: FastifyRequest, reply: FastifyReply) => {
      const query = request.query as { setup_action?: string; installation_id?: string; state?: string };

      // If this is a callback from GitHub after installation
      if (query.setup_action === "install" && query.installation_id) {
        const installationId = parseInt(query.installation_id, 10);
        if (isNaN(installationId)) {
          return reply.redirect(
            `${process.env.FRONTEND_URL}/dashboard?error=invalid_installation`,
          );
        }

        // Record this installation if we have a logged-in user
        if (request.session?.userId) {
          try {
            await fastify.prisma.githubAppInstallation.upsert({
              ...
              create: {
                ...
                accountId: BigInt(0), // Will be updated by webhook
            ...
            fastify.log.warn({
              msg: "Could not record installation (will be captured by webhook)",
        ...
        return reply.redirect(
          `${process.env.FRONTEND_URL}/dashboard?github_app_installed=true&installation_id=${installationId}`,
        );
      }
  ...
  /**
   * GET /app/installations — List all GitHub App installations for the current user.
   * Returns installation data with repo lists, scan history, and check run status.
   */
  fastify.get(
    "/app/installations",
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        return reply.send({
          ...
          data: {
            installations: installations.map((inst: any) => ({
              ...
              repositories: inst.installedRepos.map((repo: any) => ({
                id: repo.id,
                name: repo.name,
        ...
        fastify.log.error({
          msg: "Failed to list installations",
          error: error instanceof Error ? error.message : "Unknown error",
        });
        return reply.status(500).send({ error: "Failed to list installations" });
      }
    },
  );

  /**
   * POST /app/scan — Trigger a scan on a GitHub App-installed repository.
   * Uses installation token (no personal access token needed).
   */
  fastify.post(
    "/app/scan",
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const { installationId, repo, branch } = request.body as {
        ...
        if (!repo) {
          return reply.status(400).send({ error: "Repository name required (owner/repo)" });
        }
        ...
        if (installationId) {
          instId = parseInt(installationId, 10);
        } else {
          // Look up installation from repo
          const appRepo = await prisma.gitHubAppRepository.findFirst({
            where: {
              fullName: repo,
              isActive: true,
            },
          ...
          if (appRepo?.installation) {
            instId = Number(appRepo.installation.installationId);
          }
        }

        // Trigger scan using webhook infrastructure
  ...
  /**
   * GET /app/connection — Unified connection status: checks both OAuth and App installations.
   * Returns which integration method is active and available repos from both sources.
   */
  fastify.get(
    "/app/connection",
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        if (userId) {
          const githubAccount = await prisma?.githubAccount?.findFirst({
            ...
            include: {
              repositories: { where: { isActive: true }, orderBy: { updatedAt: "desc" } },
            },
          });
          if (githubAccount) {
            oauthConnected = true;
            oauthUsername = githubAccount.username;
            oauthRepos = githubAccount.repositories.map((r: any) => ({
              id: r.id,
              name: r.name,
              fullName: r.fullName,
        ...
        const installations = await prisma?.githubAppInstallation?.findMany({
          where: { isActive: true },
          include: {
            installedRepos: { where: { isActive: true } },
          },
        }) ?? [];

        const appRepos = installations.flatMap((inst: any) =>
          inst.installedRepos.map((r: any) => ({
            id: r.id,
            name: r.name,
            fullName: r.fullName,
            isPrivate: r.isPrivate,
        ...
        // Merge and dedupe repos (prefer app source)
        const seen = new Set<string>();
        const allRepos = [...appRepos, ...oauthRepos].filter((r) => {
          ...
          data: {
            ...
            app: {
              ...
              installations: installations.map((i: any) => ({
                ...
                repoCount: i.installedRepos.length,
              })),
            },
        ...
        fastify.log.error({
          msg: "Connection status error",
          error: error instanceof Error ? error.message : "Unknown error",
        });
        return reply.status(500).send({ error: "Failed to get connection status" });
      }
    },
  );
